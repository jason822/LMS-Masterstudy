...
STM_LMS_Cart::init();
	...
	public static function purchase_courses()
	{
		...
		/*Create ORDER*/
		$invoice = STM_LMS_Order::create_order($user_id, $cart_items, $payment_code, true);

        do_action('order_created', $user_id, $cart_items, $payment_code);
		...
		if ($payment_code == 'paypal') {
			...
			if (!empty($_GET['token_id'])) {
				...
				$args = array(
					...
					'amount'      => floatval($cart_total['total']) * 100,
				...
				if (!empty($req['paid']) and !empty($req['amount']) and $req['amount'] == $cart_total['total'] * 100) {
