...
class STM_LMS_Instructor extends STM_LMS_User
{
	...
	public static function meta_cap($caps, $cap, $user_id, $args)
	{
		...
		elseif ('delete_stm_lms_post' == $cap) {
			if ($user_id == $post->post_author)
				$caps[] = $post_type->cap->delete_posts;
			else
				$caps[] = $post_type->cap->delete_others_posts;
		}
		elseif ('read_stm_lms_post' == $cap) {
	...
	{
		$user = parent::get_current_user($user_id, true);
	...
	public static function my_rating($user = '') {
		...
		if ( false === ( $ratings = get_transient( $transient_name ) ) ) {
			...
			if($q->have_posts()) {
				...
				$ratings['average'] = round($ratings['total'] / $ratings['total_marks'], 2);
			...
			set_transient( $transient_name, $ratings, 24*60*7 );
		}
